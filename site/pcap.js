 ...
 *
 * Understands: libpcap file format (not pcapng), Ethernet II (including raw
 * frames with an experimental EtherType), ARP, IPv4, ICMP, IPv6, ICMPv6 (ping,
 * router and neighbour discovery), UDP (with DNS, DHCP and TFTP decoding),
 * TCP (with HTTP first-line decoding).
 *
...
(function (root) {
  ...
  /* IPv6 address in the short form of RFC 5952: lower case, leading zeros
   * dropped, the longest run of zero groups (two or more) written as :: */
  function ip6(b, o) {
    var g = [];
    for (var i = 0; i < 8; i++) g.push(((b[o + i * 2] << 8) | b[o + i * 2 + 1]));
    var best = -1, bestLen = 0, run = -1, runLen = 0;
    for (var j = 0; j <= 8; j++) {
      if (j < 8 && g[j] === 0) { if (run < 0) { run = j; runLen = 0; } runLen++; }
      else { if (runLen > bestLen && runLen >= 2) { best = run; bestLen = runLen; } run = -1; runLen = 0; }
    }
    var parts = g.map(function (x) { return x.toString(16); });
    if (best < 0) return parts.join(':');
    var head = parts.slice(0, best).join(':'), tail = parts.slice(best + bestLen).join(':');
    return head + '::' + tail;
  }
  ...
  function decodeEthernet(b) {
    ...
    var typeName = { 0x0800: 'IPv4', 0x0806: 'ARP', 0x86dd: 'IPv6', 0x88b5: 'Experimental', 0x88b6: 'Experimental' }[type] || 'Unknown';
    var dstHint = dst === 'ff:ff:ff:ff:ff:ff' ? '  (broadcast: everyone on the LAN)'
      : dst.indexOf('33:33:') === 0 ? '  (IPv6 multicast: every card listening to this group)'
      : dst.indexOf('01:00:5e:') === 0 ? '  (IPv4 multicast)' : '';
    var p = {
      src: src, dst: dst, proto: typeName, info: 'Ethertype ' + hex(type, 4),
      details: [section('Ethernet II', [
        ['Destination MAC', dst + dstHint],
        ['Source MAC', src],
        ['Type', typeName + ' (' + hex(type, 4) + ')' + (type === 0x88b5 || type === 0x88b6 ? '  - reserved by the IEEE for local experiments, so nothing else will misread it' : '')]
      ])]
    };
    p.srcMac = src; p.dstMac = dst; p.etherType = type;
    if (type === 0x0806) decodeArp(p, b, 14);
    else if (type === 0x0800) decodeIPv4(p, b, 14);
    else if (type === 0x86dd) decodeIPv6(p, b, 14);
    else decodeRaw(p, b, 14, typeName);
    return p;
  }

  /* A frame carrying anything the parser does not know, including the
   * experimental EtherType used on the Frame row to send text MAC to MAC. */
  function decodeRaw(p, b, o, typeName) {
    var end = b.length;
    /* trailing zero bytes are Ethernet padding up to the 60-byte minimum */
    var last = end; while (last > o && b[last - 1] === 0) last--;
    var n = last - o, text = isPrintable(b, o, n) && n > 0 ? ascii(b, o, n) : null;
    p.proto = typeName === 'Experimental' ? 'Ethernet' : typeName;
    p.rawData = b.slice(o, last);
    if (text !== null) {
      p.info = n + ' bytes of data, no IP inside: ' + JSON.stringify(text);
      p.textPayload = { label: 'The ' + n + ' data bytes, straight after the 14-byte Ethernet header', bytes: p.rawData };
    } else {
      p.info = n + ' bytes of data after the Ethernet header';
    }
    var rows = [['Length', n + ' bytes' + (end - last > 0 ? ' plus ' + (end - last) + ' bytes of zero padding' : '')]];
    if (text !== null) rows.push(['As text', text]);
    rows.push(['Addresses inside', 'none. The MAC addresses in the Ethernet header are the only addresses in this frame']);
    p.details.push(section('Data', rows));
  }
  ...
  var NEXT_HEADER = { 0: 'Hop-by-Hop Options', 1: 'ICMP', 6: 'TCP', 17: 'UDP', 43: 'Routing', 44: 'Fragment', 58: 'ICMPv6', 59: 'No Next Header' };

  function ip6Hint(a) {
    if (a === '::') return '  (unspecified: no address yet)';
    if (a === '::1') return '  (loopback)';
    if (a === 'ff02::1') return '  (all nodes on this link)';
    if (a === 'ff02::2') return '  (all routers on this link)';
    if (a.indexOf('ff02::1:ff') === 0) return '  (solicited-node multicast: only cards whose address ends in these 24 bits listen)';
    if (a.indexOf('ff') === 0) return '  (multicast)';
    if (a.indexOf('fe80:') === 0) return '  (link-local: valid on this LAN only, made automatically)';
    if (a.indexOf('2001:db8:') === 0) return '  (global: reachable from anywhere, given out by the router)';
    if (/^f[cd]/.test(a)) return '  (unique local: private, like 192.168.x.x)';
    if (/^[23]/.test(a)) return '  (global)';
    return '';
  }

  function decodeIPv6(p, b, o) {
    var v = b[o] >> 4, tc = ((b[o] & 0x0f) << 4) | (b[o + 1] >> 4), flow = ((b[o + 1] & 0x0f) << 16) | u16(b, o + 2);
    var payLen = u16(b, o + 4), next = b[o + 6], hop = b[o + 7];
    var src = ip6(b, o + 8), dst = ip6(b, o + 24);
    var nextName = NEXT_HEADER[next] || ('protocol ' + next);
    p.src = src; p.dst = dst; p.proto = nextName === 'ICMPv6' ? 'ICMPv6' : 'IPv6'; p.ttl = hop;
    p.info = nextName;
    p.details.push(section('Internet Protocol version 6', [
      ['Version', String(v)],
      ['Source address', src + ip6Hint(src)],
      ['Destination address', dst + ip6Hint(dst)],
      ['Next header', nextName + ' (' + next + ')  - the same job as the IPv4 protocol field'],
      ['Hop limit', hop + '  - the same job as the IPv4 TTL; each router subtracts one'],
      ['Payload length', payLen + ' bytes  - what follows the fixed 40-byte header'],
      ['Traffic class / flow label', tc + ' / ' + hex(flow, 5)],
      ['Header checksum', 'none. IPv6 has no header checksum; Ethernet and ICMPv6/TCP/UDP check the bytes instead']
    ]));
    var start = o + 40, end = Math.min(b.length, start + payLen);
    if (next === 58) decodeIcmp6(p, b, start, end);
    else if (next === 17) decodeUdp(p, b, start, end);
    else if (next === 6) decodeTcp(p, b, start, end);
  }

  function icmp6Options(b, o, end, rows) {
    var names = { 1: 'Source link-layer address', 2: 'Target link-layer address', 3: 'Prefix information', 4: 'Redirected header', 5: 'MTU' };
    while (o + 8 <= end) {
      var t = b[o], len = b[o + 1] * 8;
      if (!len) break;
      var name = names[t] || ('option ' + t);
      if (t === 1 || t === 2) rows.push([name + ' (option ' + t + ')', mac(b, o + 2) + (t === 1 ? '  - "and here is my MAC, so you do not need to ask"' : '  - the MAC that answers for the target address')]);
      else if (t === 5) rows.push([name + ' (option 5)', u32(b, o + 4) + ' bytes']);
      else if (t === 3) {
        var plen = b[o + 2], fl = b[o + 3], valid = u32(b, o + 4), pref = u32(b, o + 8);
        rows.push(['Prefix (option 3)', ip6(b, o + 16) + '/' + plen + '  - the network part of the address; make your own address from it (A flag = ' + ((fl & 0x40) ? 'yes, SLAAC' : 'no') + ')']);
        rows.push(['Prefix lifetimes', 'valid ' + valid + ' s, preferred ' + pref + ' s' + ((fl & 0x80) ? ', on-link' : '')]);
      } else rows.push([name, len + ' bytes']);
      o += len;
    }
  }

  function decodeIcmp6(p, b, o, end) {
    var type = b[o], code = b[o + 1];
    var names = { 1: 'Destination unreachable', 2: 'Packet too big', 3: 'Time exceeded', 4: 'Parameter problem',
      128: 'Echo (ping) request', 129: 'Echo (ping) reply', 130: 'Multicast listener query', 131: 'Multicast listener report', 132: 'Multicast listener done',
      133: 'Router solicitation', 134: 'Router advertisement', 135: 'Neighbor solicitation', 136: 'Neighbor advertisement', 137: 'Redirect', 143: 'Multicast listener report v2' };
    var name = names[type] || ('ICMPv6 type ' + type);
    var rows = [['Type', name + ' (' + type + ')'], ['Code', String(code)]];
    p.proto = 'ICMPv6';
    p.icmp6 = { type: type };
    if (type === 128 || type === 129) {
      var id = u16(b, o + 4), seq = u16(b, o + 6), dataLen = end - (o + 8);
      p.info = name + '  id=' + id + ', seq=' + seq + ', hop limit=' + p.ttl;
      rows.push(['Identifier', id + ' (' + hex(id, 4) + ')  - same for every ping in one run of the ping command']);
      rows.push(['Sequence number', seq + '  - goes up by one for each ping sent']);
      rows.push(['Data', dataLen + ' bytes of filler that the reply must echo back unchanged']);
      rows.push(['Compared with IPv4 ping', 'identical idea: types 128/129 instead of 8/0, and the checksum also covers the IPv6 addresses']);
    } else if (type === 133) {
      p.info = 'Router Solicitation from ' + p.srcMac + '  - "is there a router on this link?"';
      rows.push(['Meaning', 'sent to ff02::2 (all routers) when an interface comes up, so it does not have to wait for the next periodic advertisement']);
      icmp6Options(b, o + 8, end, rows);
    } else if (type === 134) {
      var curHop = b[o + 4], fl = b[o + 5], life = u16(b, o + 6), reach = u32(b, o + 8), retrans = u32(b, o + 12);
      p.info = 'Router Advertisement from ' + p.srcMac + '  - "I am a router, here is your prefix"';
      rows.push(['Router lifetime', life + ' s  - use me as default gateway for this long' + (life === 0 ? ' (0 = not a default router)' : '')]);
      rows.push(['Flags', ((fl & 0x80) ? 'M (use DHCPv6 for addresses) ' : '') + ((fl & 0x40) ? 'O (use DHCPv6 for other info) ' : '') + (!(fl & 0xc0) ? 'none: make your own address from the prefix (SLAAC), no DHCP needed' : '')]);
      rows.push(['Suggested hop limit', String(curHop)]);
      rows.push(['Reachable / retransmit timers', reach + ' ms / ' + retrans + ' ms (0 = unspecified)']);
      icmp6Options(b, o + 16, end, rows);
    } else if (type === 135) {
      var tgt = ip6(b, o + 8);
      p.info = 'Neighbor Solicitation for ' + tgt + (p.dst.indexOf('ff02::1:ff') === 0 ? '  - "who has this address?" (the IPv6 version of an ARP request)' : '  - "are you still there?"');
      rows.push(['Target address', tgt + '  - the address whose MAC we want']);
      rows.push(['Sent to', p.dst + ' = solicited-node multicast built from the last 24 bits of the target, so almost nobody else has to wake up']);
      icmp6Options(b, o + 24, end, rows);
    } else if (type === 136) {
      var f = b[o + 4], tgt2 = ip6(b, o + 8);
      var flags = [];
      if (f & 0x80) flags.push('R (I am a router)');
      if (f & 0x40) flags.push('S (answering a solicitation)');
      if (f & 0x20) flags.push('O (override any cached entry)');
      p.info = 'Neighbor Advertisement: ' + tgt2 + ' is at ' + p.srcMac + '  (the IPv6 version of an ARP reply)';
      rows.push(['Target address', tgt2]);
      rows.push(['Flags', flags.length ? flags.join(', ') : 'none']);
      icmp6Options(b, o + 24, end, rows);
    } else {
      p.info = name;
    }
    p.details.push(section('Internet Control Message Protocol v6', rows));
  }
